...
> *"The fastest code with the fewest bugs is the code that isn't written or that stays simple."*

1. **Avoid premature optimization**: Don't wrap every variable or function in `useMemo`/`useCallback` by default. The memory cost and cognitive overhead often outweigh the supposed benefit.
...
3. **Small, single-responsibility components**: Max ~200 lines per component. If it grows, extract logic into custom hooks or presentation subcomponents.
4. **Ban `class` and `this`**: 100% functional code with strict TypeScript.

---

## 2. Diagnostic and Judicious Use of Hooks

### A. `useEffect`: When to Use and When to AVOID?

`useEffect` is an escape hatch for syncing with **external systems**. It is **NOT** a mechanism for handling user events or synchronizing internal state.
...
| **Subscribe to external events (global DOM, WebSockets, Canvas, Timers)** | ✅ **YES (With Cleanup)** | External synchronization with a mandatory cleanup function in the `return`. |
...
#### ✅ Clean Solution: Render Calculation
```tsx
...
| :--- | :---: | :--- |
| Simple operations (`a + b`, `.length`, `toLowerCase()`) | ❌ **NO** | The cost of creating closures and checking dependencies exceeds the calculation. |
...
#### ✅ Legitimate Use: High-volume filtering
```tsx
...
`useCallback` preserves a **function's reference** between renders.
...
| Callbacks passed to regular child components (not wrapped in `React.memo`) | ❌ **NO** | The child will re-render anyway when the parent changes. |
...
#### ❌ Anti-pattern: `useCallback` in non-memoized components
```tsx
// BAD: Unnecessary memory consumption and noisy syntax
...
export const DataTable = ({ items }: TableProps) => {
  // GOOD: Maintains stable reference to avoid invalidating MemoizedRow's memo
...
## 4. Strict TypeScript & Clean Typing
...
- [ ] **TypeScript**: Zero `any` and strict types across all props and function signatures?
