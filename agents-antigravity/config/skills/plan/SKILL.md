...
- **The Architect (`@sheldon-architect`)**: Evaluates requirements, models system architecture, and authors the technical blueprint.
- **The Master Builder (`@profesor-orchestrator`)**: Coordinates the overall project strategy, context initialization, and phase transitions.
- **The Senior Worker (`@homero-worker`)**: Executes construction across Frontend, Backend, Mobile, Go, Rust, Python, and Infrastructure with Clean Code.
- **The UI/UX Specialist (`@edna-ux`)**: Shapes design systems, layout wireframes, tokens, and presentation components.
- **The Inspector (`@tio-bob`)**: Performs strict read-only reviews of code diffs and verification gates.
