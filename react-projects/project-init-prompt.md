...
❌ **DO NOT** install or use Tailwind CSS unless explicitly requested.
❌ **DO NOT** use CSS-in-JS libraries (styled-components, emotion, etc.) unless explicitly requested.
✅ **DO** use CSS Modules (`.module.css`) for component-specific styles.
✅ **DO** use global CSS for design system tokens and utilities.
...
3. **Tailwind CSS** (ONLY IF EXPLICITLY REQUESTED)
   ...
   - ✅ **IF used, MUST be configured to use our color variables**
...
```css
/* Button.module.css - ✅ CORRECT APPROACH */
.button {
  background-color: var(--color-primary);
  color: var(--color-white);
  padding: 0.5rem 1rem;
...
### 🔧 Tailwind CSS Configuration (ONLY IF USED)

**⚠️ WARNING**: Tailwind should NOT be installed unless explicitly requested. If you must use it, follow these STRICT rules:
...
```tsx
// ✅ CORRECT - Using our mapped colors
...
3. ✅ CSS color system is implemented correctly (no generic variable names)
4. ✅ Husky pre-commit and pre-push hooks are working
5. ✅ Biome linting passes without errors
6. ✅ TypeScript strict mode is enabled and compiling without errors
7. ✅ All dev dependencies are installed
8. ✅ Package manager is set to pnpm
9. ✅ Git repository is initialized with proper .gitignore
10. ✅ Project runs successfully with `pnpm dev`
