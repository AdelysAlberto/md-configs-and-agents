...
description: Master guide and engineering standards for modern React (18/19+) and TypeScript. Emphasizes simplicity over complexity, pure functional code, judicious hook usage (useEffect, useMemo, useCallback), real performance patterns (virtualization, code-splitting), Clean Architecture and anti-pattern prevention.
---

# React & TypeScript: Clean Architecture, Simplicity & Performance

Engineering guide for developing frontend applications in React with TypeScript. Prioritizes **radical simplicity**, functional predictability, and measurable performance over over-engineering and premature optimization.

---

## 🧭 1. Guiding Principle: Simplicity over Complexity

> *"The fastest and least buggy code is the code that isn't written or the code that stays simple."*
...
2. **Prioritize pure functions**: Extract calculation logic out of React components into easy-to-test pure TypeScript functions.
3. **Small components with a single responsibility**: Max ~200 lines per component. If it grows, extract logic into custom hooks or presentation subcomponents.
4. **Prohibition of `class` and `this`**: 100% functional code with strict TypeScript.

---

## 🔍 2. Hooks Diagnosis & Judicious Use

### A. `useEffect`: When to Use and When to AVOID?

`useEffect` is an escape hatch to synchronize with **external systems**. It is **NOT** a mechanism for handling user events or synchronizing internal state.
...
| **Handle user actions (clicks, submit)** | ❌ **NEVER** | Run the logic inside the event handler (`onClick`, `onSubmit`). |
| **Manual Data Fetching** | ❌ **AVOID** | Use **TanStack Query** (handles cache, deduplication, abort, and retries). |
| **Subscribe to external events (global DOM, WebSockets, Canvas, Timers)** | ✅ **YES (With Cleanup)** | External sync with mandatory cleanup function in the `return`. |
...
#### ✅ Clean Solution: Calculation at Render
```tsx
...
#### ✅ Legitimate Use: External Sync with Cleanup
```tsx
...
| :--- | :---: | :--- |
| Simple operations (`a + b`, `.length`, `toLowerCase()`) | ❌ **NO** | The cost of creating closures and checking dependencies is higher than the calculation. |
| Small arrays (< 500 elements) | ❌ **NO** | Modern CPUs process thousands of operations in less than 0.1ms. |
| Heavy filtering/sorting (> 1,000 elements or O(n²) transformations) | ✅ **YES** | Avoids freezing the main thread on recurring renders. |
| Preserve referential equality of objects passed to `React.memo` or hook dependencies | ✅ **YES** | Avoids invalidating the memoization of optimized child components. |

#### ❌ Anti-pattern: Unnecessary trivial memoization
```tsx
// BAD: Over-engineering for a near-instant operation
...
| :--- | :---: | :--- |
| Callbacks passed to native HTML elements (`<button onClick={...}>`) | ❌ **NO** | HTML elements are always re-evaluated in the virtual DOM; no benefit. |
...
| Callbacks passed to child components optimized with `React.memo` | ✅ **YES** | Prevents the child's `React.memo` from being invalidated on every parent render. |
| Functions that are part of the dependency array of a `useEffect` or custom hook | ✅ **YES** | Prevents the effect from running on every render cycle. |

#### ❌ Anti-pattern: `useCallback` in components without memoization
```tsx
// BAD: Unnecessary memory expenditure and noisy syntax
...
#### ✅ Solution: Standard straightforward function
```tsx
...
export const DataTable = ({ items }: TableProps) => {
  // GOOD: Keeps the reference stable so it does not invalidate MemoizedRow's memo
...
### B. Route-based Code-Splitting with `React.lazy`
Split the main bundle by loading heavy routes and components on demand.
...
- [ ] **Simplicity**: Was derived state calculated during render instead of using a `useEffect`?
...
- [ ] **TypeScript**: Zero `any` and strict types on all props and function signatures?
