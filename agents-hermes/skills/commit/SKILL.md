...
# Skill: Commit & Intelligent Staging

This skill activates when the user types `/commit` or `/git-add`, or requests to prepare and record changes in Git.

## Execution Instructions

1. **Review Git Status and Branch**:
   - Get the current branch: `git branch --show-current`.
   - Extract the 6-digit ticket ID from the branch name (example: if the branch is `feature/264099` or `264099-fix-grid`, the ID is `#264099`). If the branch does not contain a 6-digit numeric ID, omit the `#ID` prefix or request clarification only if essential.
...
4. **Draft the Commit Message**:
   - **Title Format (MANDATORY)**:
     - Must begin obligatorily with the ID preceded by `#` followed by a space (e.g: `#264099 fix(DatePicker): fix timezone offset` or `#264099 feat(auth): add refresh token support`).
   ...
   - **Commit Body (Structured Technical Description)**:
     ...
     - **Strict rule**: **LISTING FILES ONE BY ONE IS PROHIBITED** in the description.
