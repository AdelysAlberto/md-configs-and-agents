...
3. **Naming and Structure**:
   ...
   - Always expose a `reset()` action to restore initial state cleanly.
...
**SECURITY WARNING**: Avoid persisting session data or tokens directly in `localStorage` without isolation, as it may cause data leaks between users on the same browser. Prefer `sessionStorage` or the project's secure wrapper (`createAppStore`).
...
- ❌ **DESTRUCTURING THE FULL STORE IS PROHIBITED**: `const { prop1, prop2 } = useStore();` (Re-renders the component on **ANY** change in the store).
- ❌ **RETURNING LITERALS WITHOUT MEMOIZATION IS PROHIBITED**: Causes a new memory pointer on every evaluation and triggers infinite loops (`Maximum update depth exceeded`).
...
## 6. Accessing State Outside React Components
