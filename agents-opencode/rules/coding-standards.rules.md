...
## 1. Core Principle: Truth Over Agreement & Radical Simplicity

- **Truth Over Agreement**: Do NOT agree with the user when an approach is technically flawed or introduces anti-patterns. Always provide the objectively superior, robust, and maintainable solution.
- **Simplicity First**: Always prioritize the simplest, most readable solution. Avoid speculative layers or unnecessary complexity.
...
- **Zero `any`**: Use specific types, explicit generics, or `unknown` with type guards.
...
## 6. Strict DRY Invariant & Reusable Pure Utilities

- **Zero Duplication**: Never duplicate system prompts, magic strings, complex regex, or validation schemas across files.
- **Pure Reusable Utilities (`src/utils/`)**: Pure helper functions (math, formatting, transformations) must reside in `src/utils/` without side effects or UI coupling.

---

## 7. Deterministic Pre-Delivery Verification Gate

Before committing or marking any task as done, verify with 100% pass:
