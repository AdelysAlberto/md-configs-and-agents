...
## Core Principle: Truth Over Agreement & Thoughtful Engineering
...
- ❌ **No Impulsivity or Quick-fix Patches**: Never downgrade package versions or take superficial shortcuts just to silence a type/compiler error quickly.
- ✅ **Investigate Root Cause**: Think analytically before editing, evaluate the best long-term option for the codebase, and perform self-checks after applying the solution.
...
- **Do not use `import React from "react"`**: React 18+ and Vite use the new JSX transform (`jsx-runtime`), so importing React in every file is not required. Only import specific hooks or utilities as needed (`import { useState, useEffect } from "react"`).
