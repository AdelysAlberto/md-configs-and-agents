---
name: zustand
description: Advanced patterns and best practices for global and UI state management with Zustand 5+. Covers functional architecture, selector hygiene with useShallow, infinite render loop prevention, modular slices, safe storage isolation (sessionStorage / createAppStore), and access outside React.
---

# Zustand 5: Best Practices & Architecture

Comprehensive guide for managing global and UI state with Zustand 5 in a predictable, functional, and high-performance way.
...
## 2. Store Creation & Safe Persistence

### A. In-Memory Store (Non-persistent)
Ideal for volatile state that should reset when the page is refreshed.
...
**SECURITY WARNING**: Avoid persisting session data or tokens directly in `localStorage` without isolation, as it can cause data leaks between users on the same browser. Prefer `sessionStorage` or the project's safe wrapper (`createAppStore`).
...
export const useWizardStore = create<WizardState>()(
  persist(
    ...
    }),
    {
      ...
      storage: createJSONStorage(() => sessionStorage), // Tab/session isolation
      partialize: (state) => ({ currentStep: state.currentStep }), // Persist only what is needed
    }
  )
);
```
...
- ❌ **DO NOT destructure the entire store**: `const { prop1, prop2 } = useStore();` (Re-renders the component on **ANY** change in the store).
- ❌ **DO NOT return literal objects/arrays without memoization**: Creates a new memory pointer on every evaluation and causes infinite loops (`Maximum update depth exceeded`).

```tsx
// ❌ BAD: Returns new reference on every render -> Potential infinite loop
...
- [ ] Does the store handle only client/UI state without duplicating the server cache?
- [ ] Do all multi-property selectors use `useShallow`?
- [ ] Do stores expose a `reset()` action for cleanup?
- [ ] Does persistence use safe storage (`sessionStorage` or explicit partitioning)?
- [ ] Are action names clear and descriptive verbs?
